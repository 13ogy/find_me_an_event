...
PC3R — Micro-projet Web (TMEs 6 à 10), Sorbonne Université.
...
L'application combine donc une source de données dynamique externe (le
catalogue OpenData de la Mairie de Paris) avec une composante sociale
(comptes, votes, commentaires) — l'utilisateur produit lui-même du contenu
sur la plateforme.
...
Le projet n'utilise pas de tâche planifiée (cron). Tous les appels aux APIs
...
côté API est efficace et la latence est acceptable (~200 ms) pour un usage
interactif.

**Limites de débit.** OpenData Paris ne documente pas de quota strict pour
les requêtes anonymes raisonnables, et ip-api accepte 45 req/min en HTTP
gratuit. Comme l'application déclenche au plus quelques requêtes par minute
et par utilisateur, on reste largement sous les limites.
...
- **CSS vanille** — un seul fichier `app.css` avec des classes
  parlantes ; pas de framework UI, l'application est suffisamment simple
  pour ne pas nécessiter Tailwind ou Bootstrap, et reste responsive grâce
  à des media queries ciblées.
...
- **Mots de passe** stockés avec `bcrypt` (coût 10) — jamais en clair.
...
métier au navigateur et permet d'ajouter facilement du cache ou des quotas
